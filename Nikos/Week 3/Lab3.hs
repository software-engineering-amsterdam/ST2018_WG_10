...
entails b a = all (\ v -> evl v a) (allVals b)
...
-- Exercise 3 (2,5 hours) --

cnf :: Form -> Form
cnf (Prop x) = Prop x
cnf (Neg(Prop x)) = Neg(Prop x)
cnf (Cnj fs) = Cnj (map cnf fs)
cnf (Dsj [f1, Cnj [f2, f3]]) = Cnj [cnf (Dsj [f1, f2]), cnf (Dsj [f1, f3])]
cnf (Dsj [Cnj [f2, f3], f1]) = Cnj [cnf (Dsj [f1, f2]), cnf (Dsj [f1, f3])]
cnf (Dsj fs) = Dsj (map cnf fs)

convertToCnf :: Form -> Form
convertToCnf = cnf . nnf . arrowfree
