...
trClos a = calcTrClos a a


calcTrClos :: Ord a => Rel a -> Rel a -> Rel a
calcTrClos a [] = []
calcTrClos a ((x,y):xs) = nub $ sort $ [(x,y)] ++ ((findAllTr [(x,y)] a) ++ (calcTrClos a (xs)))

findAllTr :: Ord a => Rel a -> Rel a -> Rel a
findAllTr a b = if (((a ++ (a @@ b)) @@ b) == (a @@ b))
                    then (a @@ b)
                    else findAllTr (a ++ (a @@ b)) b
--------------------

-- Exercise 8 (15 minutes) --

-- (R^+)' is not equal to (R')^+
-- Below there is a counter example
--
-- R = {(1,2) , (2,3)}
--
-- R^+ =    {(1,2), (1,3) , (2,3)}
...
--
--
-- Below we can the above counter example which proves that:
-- the symmetric closure of the transitive closure of a relation R is not equal to
-- the the transitive closure of the symmetric closure of R
--
-- Lecture4> symClos (trClos [(1,2) , (2,3)])
-- [(1,2),(1,3),(2,1),(2,3),(3,1),(3,2)]
--
-- Lecture4> trClos (symClos [(1,2) , (2,3)])
